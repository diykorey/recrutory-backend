...
- This directory is a **near-duplicate of `../recrutory-backend`** — the same `com.kandidato` Maven app (Java Spring backend + Backbone.js UI in one WAR). Keep changes in sync with it, or confirm with the user which copy is canonical before diverging. See the root `../CLAUDE.md`.
- **Ignore `.svelte-kit/`, `node_modules/`, and `webapps/.gitkeep`.** There is **no `package.json` or svelte config** anywhere here — SvelteKit is *not* used. The real frontend is the Backbone.js code in `kandidato-web/src/main/webapp/js/`.

## Overview

**Kandidato** — a recruitment / candidate-management (ATS) web app. A full-stack application: a Java Spring backend with a Backbone.js single-page UI bundled into the same WAR.
